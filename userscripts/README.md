...
4. 点击右下角“导出课表 Excel”，在弹窗日历中选择开始与结束日期，再选择导出模式。

脚本会从 GitHub 自动检查更新；日后不需要再次复制粘贴。

## 与 Python 版的差异

“完整导出”会生成“课表”明细表，以及每个月一张固定五时段的月视图；“极速导出”只生成明细表，适合需要尽快筛选数据的场景。详情请求最多三路并发，以兼顾速度与接口稳定性。它不会保存账号或密码；请求直接使用你当前浏览器已登录页面的会话。
